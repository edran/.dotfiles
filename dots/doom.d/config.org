...
dark -- and \tilde{}cool\tilde{} -- experience.
...
By default, we display line numbers inline for most text buffers. However not
only am I not particularly interested in having them when I'm using org-mode
(due to it coming with an already powerful navigation system), but they also
don't behave well in buffers with variable-size fonts.

So we disable them accordingly. Voilà.

#+begin_src emacs-lisp
(defun nn/no-line-numbers ()
  (setq-local display-line-numbers nil))
(add-hook 'org-mode-hook 'nn/no-line-numbers)
#+end_src
...
Sadly we can't do this as that would break =persp-mode=. There's an [[github:Bad-ptr/persp-mode.el/issues/104][issue]] that
tracks this problem.
...
even sillier is that ~ivy-immediate-done~ is relegated to the default keybinding
