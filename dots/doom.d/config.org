...
*** Archiving into DONE file

Managing DONE issues is annoying, because I tend to be messy and spread them across multiple files. Let's set a single "archiving" file:

#+begin_src emacs-lisp
(defvar nn/org-done-file (concat org-directory "done.org")
  "Path to DONE file.")
#+end_src

We also need to automate the boring work:

#+begin_src emacs-lisp
(defun nn/org-archive-to-done (&optional date)
  "Archive the current subtree to `nn/org-done-file'.

If DATE is a string like \"2025-06-29\" use it; otherwise default
to today.  Creates (or reuses) a level‑1 heading ‘* [DATE]’ and
puts the subtree beneath it."
  (interactive)
  (let* ((date (or date (format-time-string "%Y-%m-%d")))
         (org-archive-location
          (format "%s::* [%s]" nn/org-done-file date)))
    (org-archive-subtree)))

(defun nn/org-archive-buffer-done ()
  "Archive all DONE or KILL headlines in the current buffer."
  (interactive)
  (let ((count 0))
    (org-map-entries
     (lambda ()
       (setq count (1+ count))
       ;; CLOSED date → 2025-06-27, else today
       (let* ((closed (org-entry-get (point) "CLOSED"))
              (date   (if (and closed
                        ...
                        nil)))
         (nn/org-archive-to-done date)))
     "TODO=\"DONE\"|TODO=\"KILL\"TODO=\"READ\"")
    (message "Archived %d finished items." count)))

(after! org
  (map! :map org-mode-map
        :localleader
        "Z" #'nn/org-archive-to-done
        "z" #'nn/org-archive-buffer-done))
#+end_src

=<leader> m z= for the buffer wide, and =Z= for the finetuned one in case I care to run it.
...
#+begin_src emacs-lisp
(after! org-modern
  (setq org-modern-fold-stars '(("一" . "一")
                                ("二" . "二")
                                ("三" . "三")
                                ("四" . "四")
                                ("五" . "五")
...
(setq org-roam-dailies-capture-templates
      `(("d" "default" entry "* %<%I:%M %p>\n%?"
        ...
        ("w" "work")
        ("wd" "default" entry "* %<%I:%M %p> :pf:\n%?"
         :target (file+head "%<%Y-%m-%d>.org" ,nn-org-roam-dailies-header)
         :jump-to-captured t
         :empty-lines-before 1)
        ("wl", "linear" entry
         "* %<%I:%M %p> [[https://linear.app/pacificfusion/issue/%^{Linear issue}][%\\1]] :pf:\n%?"
         :target (file+head "%<%Y-%m-%d>.org" ,nn-org-roam-dailies-header)
         :jump-to-captured t
         :empty-lines-before 1)
        ("wm" "meeting" entry "* %<%I:%M %p> %? :pf:meeting:"
         :target (file+head "%<%Y-%m-%d>.org" ,nn-org-roam-dailies-header)
         :jump-to-captured t
         :empty-lines-before 1)))
#+end_src
...
(after! org-roam
  (map! :leader
        ...
        (:prefix ("j" . "journal")
         ...
         :desc "Today's journal" "t" (cmd! (org-roam-dailies-goto-today "d"))
         :desc "Tomorrow's journal" "T" (cmd! (org-roam-dailies-goto-tomorrow 1 "d"))
         :desc "Yesterday's journal" "y" (cmd! (org-roam-dailies-goto-yesterday 1 "d"))
         :desc "Open journal at date" "d" (cmd! (org-roam-dailies-goto-date "d")))))
#+end_src
